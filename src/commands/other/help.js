    ...
    run: async (bot, message, args) => {
        if(args[0]) {
            ...
            let Sembed = new RichEmbed()
                ...
                .addField(`Commands:`, "``8ball`` ``cat`` ``dog`` ``meme`` ``addrole`` ``ban`` ``kick`` ``mute`` ``purge`` ``removerole`` ``unmute`` ``warn`` ``botinfo`` ``helpmsg`` ``help`` ``ping`` ``serverinfo`` ``urban`` ``say`` ``welcome`` ``roast`` ``anime`` ``pat`` ``hug`` ``slap`` ``punch`` ``warn`` ``warnlevel`` ``tickle`` ``dadjoke`` ``tempmute`` ``uptime`` ``spank`` ``clearwarns(not working)`` ``vote``");
